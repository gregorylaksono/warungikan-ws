...
public class UserManagerImpl  {
	...
	public User getUserAsUser(String sessionId,String userId)  throws UserSessionException,WarungIkanNetworkException{
		try {
			RestTemplate r = new RestTemplate();
			HttpHeaders headers = new HttpHeaders();
			headers.add("Authorization", sessionId);
			HttpEntity request = new HttpEntity<>(headers);
			ResponseEntity<User> response = r.exchange(new URI(Constant.WS_CHECK_USER_AS_USER_URL+"/"+userId),HttpMethod.GET, request, User.class);
			if(response.getStatusCode() == HttpStatus.OK){
				return response.getBody();			
			}else{
				return null;
